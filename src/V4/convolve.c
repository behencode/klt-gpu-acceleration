  ...
  _KLT_FloatImage imgout)
{
  ...
  /* OpenACC: parallelize row loop for GPU execution (T4) */
  #pragma acc parallel loop gang num_gangs(128) vector_length(32)
  /* For each row, do ... */
  for (j = 0 ; j < nrows ; j++)  {
  ...
  _KLT_FloatImage imgout)
{
  ...
  /* OpenACC: parallelize column loop for GPU execution (T4) */
  #pragma acc parallel loop gang num_gangs(128) vector_length(32)
  /* For each column, do ... */
  for (i = 0 ; i < ncols ; i++)  {
